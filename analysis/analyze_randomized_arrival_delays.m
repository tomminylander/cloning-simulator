...
if ~isfile('randomized_arrival_data.mat')
    ...
    if ~(check_result_path(path))
        path = '../prerun-simulation-results/randomized-arrival-delays';
...
fileID = fopen([dataPath 'randomized_arrival_delays_confint_bound.txt'],'w');
fprintf(fileID,'%6.4f %6.4f %6.4f %6.4f\n',(conf_bound_errors)');
fclose(fileID);

fileID = fopen([dataPath 'randomized_arrival_delays_confint_resp.txt'],'w');
fprintf(fileID,'%6.4f %6.4f %6.4f %6.4f\n',(conf_resp_errors)');
fclose(fileID);
